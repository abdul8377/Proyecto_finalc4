    ...
    public function agregar($producto){
        ...
        if(!$this->carro){
            $this->carro=[$producto->id=>[
                    "name"=>$producto->name,
                    "amount"=>1,
                    "price"=>$producto->price_discount,
                    "image"=>$imagen,
                    "total"=>$producto->price_discount
                ]
            ];
        //Si el carro existe y el producto tambien, sumamos mas 1 la cantidad
        }else if(isset($this->carro[$producto->id])){
            $this->carro[$producto->id]['amount']++;
            $this->carro[$producto->id]['total']=$this->carro[$producto->id]['subtotal']+$this->carro[$producto->id]['precio'];
